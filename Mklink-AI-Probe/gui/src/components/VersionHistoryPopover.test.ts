...
describe('VersionHistoryPopover', () => {
  it('shows the current release notes and stable release history', async () => {
    ...
    expect(panel.text()).toContain('修复热插拔重连、RTT 高频显示、Pack 与型号解析')
    ...
    expect(panel.text()).toContain('可逆读保护加锁与解锁')
    expect(panel.text()).toContain('未验证器件保持置灰')
    expect(panel.text()).toContain('勾选时立即确认')
    expect(panel.text()).toContain('断电复位')
    expect(panel.text()).toContain('精确订货型号与内置通用型号')
    expect(panel.text()).toContain('STM32F413/423')
    expect(panel.text()).toContain('完整保留 USER 与写保护选项')
    ...
    expect(wrapper.get('.release-entry.current').findAll('li')).toHaveLength(8)
