...
defineProps<{ targets: TargetRecord[]; selectedPart: string; status: PackStatus | null; busy: boolean; progress: number; error: string }>()
...
<template>
  <section class="target-panel">
    ...
    <div class="target-list">
      <button v-for="target in targets" :key="target.part_number" :data-testid="`target-${target.part_number}`" :class="{ active: selectedPart === target.part_number }" @click="emit('select', target)">
        <strong>{{ target.part_number }}</strong><small>{{ target.vendor }} · {{ target.pack_id || '内置' }}</small><span>{{ target.installed ? '已安装' : '需下载 Pack' }}</span>
      </button>
    </div>
    <div v-if="busy" class="pack-progress"><progress :value="progress" max="1"/><span>{{ Math.round(progress * 100) }}%</span><button @click="emit('cancel')">取消</button></div>
