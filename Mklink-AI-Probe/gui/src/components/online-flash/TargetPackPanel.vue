...
<template>
  <section class="target-panel">
    ...
    <div ref="searchBox" class="target-combobox" @focusout="onSearchFocusOut">
      <input v-model="query" data-testid="target-search" type="search" role="combobox" aria-autocomplete="list" aria-controls="target-suggestions" :aria-expanded="suggestionsOpen" :aria-activedescendant="suggestionsOpen && activeSuggestion >= 0 ? `target-option-${activeSuggestion}` : undefined" :placeholder="tr('搜索型号 / 厂商 / 系列', 'Search model / vendor / family')" :aria-label="tr('搜索器件', 'Search targets')" @input="onSearchInput" @focus="onSearchFocus" @keydown="onSearchKeydown">
