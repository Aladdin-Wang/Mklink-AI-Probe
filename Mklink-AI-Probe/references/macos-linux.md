...
- WebGUI 的本机路径来源会检查内容变化：HEX/BIN 重新检查并刷新预览；AXF/MAP
  ...
  浏览器授予持续文件访问权限时，可以监测所选固件；普通文件上传只是一份快照，
  无权追踪原文件，编译后须重新选择，或使用本机文件路径/桌面版。
  在线烧录可点击“文件路径”，填写运行后端的电脑能访问的 HEX/BIN 路径，再点击
  “加载并跟踪”。Mac 后端应填写 Mac 共享目录路径，不填写虚拟机的 Windows 盘符。
