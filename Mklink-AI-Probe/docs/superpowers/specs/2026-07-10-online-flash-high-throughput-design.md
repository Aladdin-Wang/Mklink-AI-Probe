...
继续以 `mklink/mcu_profiles.json` 为芯片公共信息源，在每个 MCU profile 中增加可选的 `cmsis_dap` 对象：

```json
{
  "cmsis_dap": {
    "enabled": true,
    "pyocd_target": "target-name",
    "pack": "packs/vendor.device.pack",
    "connect_mode": "halt",
    "reset_type": "hardware"
  }
}
...
公共字段继续复用 `flash_base`、`flash_size`、`ram_base`、`sector_size`、`page_size`、`regions` 和 `swd_clock_default`。未声明 `cmsis_dap.enabled` 或缺少可用 target/Pack 的 MCU 不出现在在线烧录页面中。

Pack 路径必须通过统一资源定位器解析，兼容源码运行和 PyInstaller/Tauri sidecar。连接前验证 Pack 是否存在，并返回稳定的 `PACK_NOT_FOUND` 错误。
...
- profile 的 `cmsis_dap` 配置解析与 Pack 定位。
...
- pyOCD、CMSIS-Pack、USB backend 和目标配置随 PyInstaller sidecar 正确收集。
...
- 新增设计/实施文档、在线烧录错误码、profile schema 和接口模型。
- 为现有 profile 增加可选 `cmsis_dap` 配置，未验证目标默认禁用。
- 建立 fake backend 和基础测试夹具。

验收：配置和错误模型单元测试通过；现有测试不回归。
...
3. 固件越界、Pack 缺失、探针忙、任务失败和停止均有明确错误与可靠清理。
4. 在线烧录与所有目标调试/数据会话通过 `target_debug` 互斥。
5. SystemView、VOFA、RTT 和 SuperWatch 使用共享的高吞吐基础设施或完成约定的迁移范围。
6. 合成数据测试达到或超过 10 kSamples/s，绘图最高 30 FPS，内存有界且丢包可观测。
7. MKLink 硬件实测记录实际最大稳定吞吐，不能用合成测试替代硬件结论。
8. 自动测试、Tauri 打包和阶段文档完整，GitHub 上有可回退的阶段提交。
