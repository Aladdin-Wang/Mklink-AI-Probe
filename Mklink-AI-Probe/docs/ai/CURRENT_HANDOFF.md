...
- 更新时间：`2026-09-03T11:40:00+08:00`
...
- 当前任务：合并 HEX 执行解析与通用复位检查修复已提交 21cebb8。STM32F103RE + V3 真实 WebGUI 接 RST 九组矩阵、拔 RST 后暂停/保持运行×默认/软件复位四组均通过烧录、校验及运行状态检查；末次下载后应用计数与心跳连续递增。485 项在线烧录 Python 与 647 项 GUI 已通过，本轮仅补真机证据，未重打包。已通知用户换 GD32 板并接回 RST，等待型号确认后验证客户 HEX；既有 DWT Trace 与 UART 问题仍待处理。
...
- **合并 HEX 与复位连接矩阵**：GD32F303xE 客户 HEX 重复 05 入口记录兼容修复；真实 FileProgrammer 离线暂存两段共 180732B 与校验数据一致。V3 接 RST 九组及拔 RST 后暂停/保持运行×默认/软件复位四组 WebGUI 真机通过，应用采样计数与心跳 139→140→141。新版硬件统一按开漏处理，不加旧硬件特例。详见 docs/verification/v0.2.0-hex-connect-modes-hil.md；仅 GD32 原客户 HEX 真机仍待换板。
...
- **probe**：当前 MKLink V3 + STM32F103RE，用户确认 RST 为开漏；新版 V4 同样采用开漏。用户已拔 RST 并完成无复位线测试，换下一块板时需确认接回。重新发现端口，不沿用原 V4 的端口号。
...
1. STM32 接 RST/无 RST 下载测试已完成；等待用户换 GD32 板并接回 RST，确认具体型号与容量后通过 WebGUI 验证客户合并 HEX，禁止把 GD32 客户固件写入 STM32。
