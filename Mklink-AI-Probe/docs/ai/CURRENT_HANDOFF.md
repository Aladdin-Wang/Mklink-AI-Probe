...
- 更新时间：`2026-09-02T18:18:00+08:00`
...
- 当前任务：在线烧录已完成 STM32F103、STM32F413/423、STM32G474xE 与 STM32H743xI 的可逆 RDP Level 1 真机闭环；精确型号读取自动使用通用内置算法，器件联想热查询降至约 9–47ms。后续安全支持按保护架构验证代表芯片后成组扩展。
- 状态：`v0.2.0-development`

## 里程碑

- **0.2.0 开发周期** — `in_progress`。预发布分支从 origin/master 的 0.1.9 已发布基线建立；已完成下载器热插拔重连、RTT/串口高频显示、不规范 CMSIS-Pack 宽松导入、常用型号内置算法、跨系列精确/通用型号解析、器件联想加速，以及 STM32 F1/F4/G4/H7 安全加锁/解锁和 VCC 断电复位。
...
- **0.2.0 本地候选**：源码提交 37b0fbb 生成标准 NSIS 91491384B（SHA-256 F7960F042AB572DBFF1B468FDD831F7851DAA1E6BFFAD6DFFB31F7A3A5EA1662；签名 SHA-256 A66A06596C2C1743AD47623756B31FF1F7B24008D2713E03C38E75D3C45A1EA5）和普通用户 Skill ZIP 19333084B（SHA-256 4BD390BE8E22B5D5A1440B2E397897601DB31941D8B1DB10B7A5629329EAAA74），均含 7059 个目标和 2224 个去重 FLM。先前候选已完成覆盖安装与下载器热插拔重连验证；本候选补入系列级型号解析、读取镜像回烧和 STM32F413/423 安全闭环，尚未授权正式发布。
...
- **STM32 安全真机 HIL**：STM32F103RE、STM32F413VGHx 与 STM32G474RET6 的加锁、保护读取拒绝、解锁擦除和镜像恢复闭环通过。STM32H743IIT6（ID 0x450、2MiB）在用户确认的 3.3V 下完成 RDP1 加锁、保护读取拒绝、复位下解锁、整片擦除、完整恢复及最终断电启动；独立 2MiB 回读 SHA-256 与原备份一致。详见 docs/verification/v0.2.0-stm32f103re-security-hil.md、v0.2.0-stm32f413-security-hil.md、v0.2.0-stm32g474-security-hil.md 与 v0.2.0-stm32h743-security-hil.md。
- **SuperWatch**：V4.3.9 下 16×float、1ms 请求得到 4690 样本，中位周期 1000us；暂停/停止后历史仍可查看。
- **自动化与构建**：GUI 全量 635 项、当前系列型号/Pack/在线烧录安全相关 Python 233 项通过，标准生产构建成功；此前 Python 全量 1693 项通过，另 12 项仅因当前 Windows 账户缺少目录 symlink 权限触发 WinError 1314。断电复位界面显示 1.8V/3.3V/5V 且默认 3.3V；STM32G474RET6 与 STM32H743IIT6 已实际完成 3.3V 断电复位安全闭环。
...
- 安全加锁/解锁按选项字节布局和保护机制划分白名单；每种架构至少用一块代表芯片完成保护、解锁擦除和恢复闭环，再用芯片 ID、容量、算法哈希及选项区范围覆盖同架构料号。只允许可逆等级并永久拒绝 RDP2。STM32G474 与 STM32H743 解锁使用复位下连接，安全操作强制使用用户确认电压的断电复位。

## 真机环境

- **probe**：主要 ARM 回归夹具为 MKLink V4；发布前验证固件 V4.3.9，调试端口 COM228、UART COM227。
- **target**：STM32F103RE 工程 E:\PHDZ\PROJECT\liu\STM32F103_test\STM32F103RC；STM32F413VGHx（ID 0x463、1MiB）、STM32G474RET6（ID 0x469、512KiB、3.3V）和 STM32H743IIT6（ID 0x450、2MiB、3.3V）均完成型号解析、读取回烧与 RDP Level 1 闭环。H743 原始/恢复读回 SHA-256 为 3E0FC3D91B03294E00E04B46EB82E1FDCEB872665F93E1BAA31A71BA53DCF614。
...
4. 后续 STM32 安全支持按保护架构挑选代表板验证，不逐一验证同架构的每个完整订货号；运行时继续校验芯片 ID、容量与固定算法。
5. 用户换接 GD32 板卡后，按同等标准验证可逆加锁、受保护读取拒绝、解锁擦除及固件恢复，再启用对应安全架构白名单。
...
- 当前 STM32 F1、F4、G4、H7 各有代表芯片完成 RDP Level 1 真机闭环；F0/F3/F7/L/U/W 等不同保护架构以及 GD32 仍保持置灰，需按架构完成至少一块代表板验证后再成组启用。
- 3.3V 断电复位已在 STM32G474RET6 与 STM32H743IIT6 安全闭环中实际验证；1.8V 与 5V 仍只在目标板明确支持对应电压时使用。
