...
- 更新时间：`2026-09-02T09:45:53+08:00`
...
- 当前任务：在线烧录已为 STM32F103 增加安全加锁/解锁，并完成 RDP Level 1 加锁、受保护读取拒绝、解锁整片擦除及 bootloader/app 恢复真机闭环；GD32 等待换板验证。
- 状态：`v0.2.0-development`

## 里程碑

- **0.2.0 开发周期** — `in_progress`。预发布分支从 origin/master 的 0.1.9 已发布基线建立；已完成下载器热插拔重连、RTT/串口高频显示、不规范 CMSIS-Pack 宽松导入、常用型号内置算法及 STM32F103 安全加锁/解锁。
...
- **0.2.0 本地候选**：源码提交 e40ea4b 生成标准 NSIS 91455283B（SHA-256 9A57795CCB83BA56D26F9C96F7BCE9408AACA1B81B2D5342B3A2B66A7310BB02）和普通用户 Skill ZIP 19321424B（SHA-256 51060FCCF8D105EB6F5C7A24CE862D83ED140C11000C96D769316C60027B3458）。真实 COM228 连接进入与 USB 拔出相同的 transport error 状态且旧句柄、端口锁仍被持有时，连接接口会关闭旧会话、释放两项资源并建立新设备对象；覆盖安装后保持桌面进程运行并真实拔插 USB，旧会话进入 ERROR、COM228 重新枚举，单次连接即恢复 READY，无需重启上位机。产品版本 0.2.0、健康状态 ok、无 Python 子进程，嵌入 WebGUI 含热插拔修复版本信息。
...
- **自动化与构建**：GUI 全量 631 项通过，安全界面与版本信息定向 86 项通过；Python 安全相关定向 257 项通过。Python 全量运行 1672 passed、14 failed，新增错误码断言已修复且相关用例与一次瞬态桌面用例复跑 18 项通过，剩余 12 项均为当前 Windows 账户无目录 symlink 权限导致的 WinError 1314；标准生产构建成功。
...
- 安全加锁/解锁仅按真机验证型号逐项启用；必须白名单校验型号、算法哈希和选项区范围，只支持可逆保护，解锁必须提示整片擦除，未验证型号前后端均拒绝。
...
4. 用户换接 GD32 板卡后，按 STM32 同等标准验证可逆加锁、受保护读取拒绝、解锁擦除及固件恢复，再决定启用的精确系列。
