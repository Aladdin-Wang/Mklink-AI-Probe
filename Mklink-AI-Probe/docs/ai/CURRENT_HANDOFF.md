...
- 更新时间：`2026-08-27T21:56:15.552622+08:00`
- 分支：`codex/v0.1.9-development`
- HEAD：`2978f02 fix(installer): preserve files and shortcuts during automatic updates（本交接更新前的已推送功能基线）`
- 远端 HEAD：`origin/codex/v0.1.9-development 已包含功能基线 2978f02；master 保持 a42f9ed，未合并或发布；交接随本提交保存，最终文档提交号以 Git 为准。`
- 工作树：版本/集中构建存储、串口自定义波特率、HPMLink V4.3.8 升级包和自动升级安装钩子修复均已提交，交接随本提交保存。旧 fix-independent-firmware-channel 工作树已安全移除，Cargo 缓存仍在主工作区 .build/cache/cargo；本机测试记录与保留目录均被 Git 忽略。
- 当前任务：已按维护者授权推送 0.1.9 开发分支，移植 PR #4 的自定义串口波特率并署名后关闭 PR；同时修复自动升级期间自定义卸载器与安装器竞争、快捷方式消失的问题。并未合并 master、签名、发布安装包或修改更新通道。
- 状态：`v0.1.9-development-pushed-release-gates-pending`
...
- 维护者授权将 codex/v0.1.9-development 推送 GitHub并在功能移植后关闭 PR #4；只删除旧开发分支/工作树，保留正式 0.1.8 Release、标签和资产。不授权签名、发布、Gitee 同步或修改更新指针。
...
- **permission**：本轮仅枚举端口，只有蓝牙串口，无 USB 串口条件；浏览器使用模拟串口接口，没有烧录、串口收发或 VCC 操作。安装流程用隔离夹具，既有正式安装保持 0.1.8。
...
5. 后续构建/测试继续使用 scripts/build_workspace.ps1，共用 .build/cache，禁止 C 盘临时构建及上传 .build；正式 0.1.8 仍是用户发布版。
