...
- 更新时间：`2026-09-02T12:42:30+08:00`
...
- **自动化与构建**：GUI 全量 633 项、系列型号/Pack/在线烧录相关 Python 233 项通过，标准生产构建成功；Python 全量 1693 项通过，另 12 项仅因当前 Windows 账户缺少目录 symlink 权限触发 WinError 1314。断电复位界面显示 1.8V/3.3V/5V 且默认 3.3V，当前单只下载器的 CMSIS-DAP 与命令口身份一一匹配；未实际切换 VCC，等待用户在界面确认具体电压后手动验证。
...
4. 用户在 WebGUI 中按目标板实际供电选择恢复电压并手动验证一次断电复位，确认关闭、1 秒延时、恢复输出和目标启动均正常。
