...
- 更新时间：`2026-09-05T15:39:29+08:00`
- 分支：`codex/v0.2.0-development`
- HEAD：`7044dd4上游同步基线之后增加pyOCD断开时跟踪时钟保留修复；生产GUI仍为ada1ca0构建。已安装本地候选d30b884不含这两轮修复。`
- 远端 HEAD：`每次维护前校正 GitHub origin/codex/v0.2.0-development。`
- 工作树：历史脱机/GUI/PY32改动已收口提交。候选包和原始证据仅在忽略的 .build；源码与生成GUI均受版本管理。
- 当前任务：F103真机复现并修复pyOCD普通在线烧录后清DEMCR导致SystemView时基停止；相同固件复烧后时钟前进，真实Edge约72秒590521事件、连续第二会话127216事件、无自动重试，暂停显示/停止/断连通过。Python定向510通过。用户接手UART排查，已释放UART和命令口；8765源码服务保留、设备未连接，不再自动占用串口。
...
1. 用户正在手动排查UART；源码服务8765保留但无设备/串口连接，不要自动重新占用。已安装d30b884不含上游和本轮pyOCD修复；后续使用最新源码或重建包。
2. 芯片安全长期任务按 docs/ai/security-roadmap.md 推進；优先PY32异常和加锁后独立运行，有对应板卡再扩展。
3. F103的SystemView长流、连续启停和普通烧录后时基已完成；后续补探针断电冷启动、其他板卡、Mac/Linux及用户反馈后的UART验证。正式发布前重建安装包并跑发布全量门禁。
...
- Mac/Linux无实机兼容验证；用户报告首次假烧录/后端退出缺少原始材料，未复现。UART有效文本接收未通过，已释放接口由用户手动排查。
