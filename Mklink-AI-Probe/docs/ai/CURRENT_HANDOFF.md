...
- 更新时间：`2026-09-05T11:04:00+08:00`
...
- 当前任务：GD32F303CET6 新程序已双读备份完整512KiB；在线加锁/解锁空白/恢复比对及V3脱机解锁/完整BIN/加锁通过，再解锁恢复后全片比对一致。最终未加锁，三次内核运行快照RUNNING、线程模式、CFSR=0，已恢复运行释放会话。用户提供的GD32F303_BOOT AXF匹配备份加载段，但没有RTT符号，未做虚假RTT验收。加锁后重新接调试仍见LOCKUP，独立运行未证明；不能只归因客户自锁。详见v0.2.0-gd32-halt-security-retest.md及本地.build/reports/gd32-new-program-20260905。本轮未改产品源码、固件或安装包，保留此前未提交修改。
...
- **probe**：当前V3 + GD32F303CET6，普通halt、3.3V测试，不主动选择RST连接。用户新程序双读备份后完成在线和脱机保护/恢复测试，最终恢复未加锁并证明内核正常执行。原始备份及记录在外层.build/reports/gd32-new-program-20260905，U盘自动脚本已改用新备份，仍包含解锁和加锁。WebGUI后端保留，探针会话已释放。换板、电压或镜像需重新核对。
...
1. GD32新程序备份与在线/脱机保护恢复已完成。下一步若继续查锁定后运行，需不依赖SWD的可观测证据，或明确授权增加非侵入运行指示；当前BOOT构建无RTT。审查在线断电后的旧会话清理、脱机加锁后的重连验证对运行的影响，不把恢复后正常执行替代锁定运行验收。
