...
- **probe**：V4 下载器，Python Console COM228、UART COM227、RS485 COM229；本轮所有下载器访问串行执行。
- **target**：STM32F103RE 测试夹具 E:\PHDZ\PROJECT\liu\STM32F103_test；AXF 为 STM32F103RC\build\keil\Obj\rt-thread.axf。RTT 控制块 0x20000B88，Up0/Up1 和 Down0/Down1 活跃，channel 2 inactive；Up1 承载真实 SystemView。
- **permission**：维护者授权任意修改、编译和下载 STM32F103_test，无需备份；本轮使用既有已下载固件完成烧录、YMODEM、RTT/SystemView、内存/寄存器、CLI/MCP/REST 边界验证，未切换目标供电。
