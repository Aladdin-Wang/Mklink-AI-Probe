...
- 更新时间：`2026-09-02T10:25:40+08:00`
...
- 当前任务：在线烧录已将 STM32F103 加锁/解锁确认提前到勾选时，并新增断电复位及 1.8V/3.3V/5V 恢复电压（默认 3.3V）；后端按探针身份精确匹配命令口，GD32 等待换板验证。
...
- **自动化与构建**：GUI 全量 633 项通过，本轮安全界面、断电复位与版本信息定向 88 项通过；Python 本轮烧录后端、作业、API、错误与探针控制定向 287 项通过。断电复位界面在真实 WebGUI 显示 1.8V/3.3V/5V 且默认 3.3V，当前单只下载器的 CMSIS-DAP 与命令口身份一一匹配；未实际切换 VCC，等待用户在界面确认具体电压后手动验证。标准生产构建成功。
...
4. 用户在 WebGUI 中按目标板实际供电选择恢复电压并手动验证一次断电复位，确认关闭、250ms 延时、恢复输出和目标启动均正常。
5. 用户换接 GD32 板卡后，按 STM32 同等标准验证可逆加锁、受保护读取拒绝、解锁擦除及固件恢复，再决定启用的精确系列。
