...
- 更新时间：`2026-09-01T19:20:00+08:00`
...
- 当前任务：已修复桌面上位机运行期间下载器热插拔后旧串口会话未释放、必须重启应用才能重连的问题，并完成 0.2.0 覆盖安装验证。
- 状态：`v0.2.0-development`

## 里程碑

- **0.2.0 开发周期** — `in_progress`。预发布分支从 origin/master 的 0.1.9 已发布基线建立；已完成下载器热插拔重连、RTT/串口高频显示、不规范本地 CMSIS-Pack 宽松导入及常用型号内置算法目录扩充。
