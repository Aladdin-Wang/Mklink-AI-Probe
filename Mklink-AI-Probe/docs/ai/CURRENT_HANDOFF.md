...
- 更新时间：`2026-09-02T16:02:00+08:00`
...
- 当前任务：在线烧录已完成 STM32F103、STM32F413/423 与 STM32G474xE 的可逆 RDP Level 1 真机闭环；STM32G474RET6 精确型号读取自动使用通用内置算法，器件联想热查询降至约 9–47ms。后续安全支持按保护架构验证代表芯片后成组扩展。
- 状态：`v0.2.0-development`

## 里程碑

- **0.2.0 开发周期** — `in_progress`。预发布分支从 origin/master 的 0.1.9 已发布基线建立；已完成下载器热插拔重连、RTT/串口高频显示、不规范 CMSIS-Pack 宽松导入、常用型号内置算法、跨系列精确/通用型号解析、器件联想加速，以及 STM32 F1/F4/G4 安全加锁/解锁和 VCC 断电复位。
...
- **STM32 安全真机 HIL**：STM32F103RE 与 STM32F413VGHx 的加锁、保护读取拒绝、解锁擦除和镜像恢复闭环通过。STM32G474RET6（ID 0x469、512KiB）在 3.3V 断电复位下完成两轮 RDP1 加锁、保护读取拒绝、连接下复位解锁、整片擦除和完整恢复；恢复后独立 512KiB 读回 SHA-256 与原备份一致，精确型号 WebGUI 读取亦一致。详见 docs/verification/v0.2.0-stm32f103re-security-hil.md、v0.2.0-stm32f413-security-hil.md 与 v0.2.0-stm32g474-security-hil.md。
- **SuperWatch**：V4.3.9 下 16×float、1ms 请求得到 4690 样本，中位周期 1000us；暂停/停止后历史仍可查看。
- **自动化与构建**：GUI 全量 634 项、当前系列型号/Pack/在线烧录安全相关 Python 228 项通过，标准生产构建成功；此前 Python 全量 1693 项通过，另 12 项仅因当前 Windows 账户缺少目录 symlink 权限触发 WinError 1314。断电复位界面显示 1.8V/3.3V/5V 且默认 3.3V；STM32G474RET6 已实际完成 3.3V 断电复位安全闭环。
...
- 安全加锁/解锁按选项字节布局和保护机制划分白名单；每种架构至少用一块代表芯片完成保护、解锁擦除和恢复闭环，再用芯片 ID、容量、算法哈希及选项区范围覆盖同架构料号。只允许可逆等级并永久拒绝 RDP2。STM32G474 解锁使用复位下连接，安全操作强制 3.3V 等用户确认电压的断电复位。

## 真机环境

- **probe**：主要 ARM 回归夹具为 MKLink V4；发布前验证固件 V4.3.9，调试端口 COM228、UART COM227。
- **target**：STM32F103RE 工程 E:\PHDZ\PROJECT\liu\STM32F103_test\STM32F103RC；STM32F413VGHx（ID 0x463、1MiB）和 STM32G474RET6（ID 0x469、512KiB、3.3V）均完成型号解析、读取回烧与 RDP Level 1 闭环。G474 原始/恢复读回 SHA-256 为 063A75F1CB15147F8B26A5FCFB406B959EDBA73A2D8B47F5C4E48F60329CEBA4。
...
- 当前 STM32 F1、F4、G4 各有代表芯片完成 RDP Level 1 真机闭环；F0/F3/F7/H7/L/U/W 等不同保护架构以及 GD32 仍保持置灰，需按架构完成至少一块代表板验证后再成组启用。
- 3.3V 断电复位已在 STM32G474RET6 安全闭环中实际验证；1.8V 与 5V 仍只在目标板明确支持对应电压时使用。
