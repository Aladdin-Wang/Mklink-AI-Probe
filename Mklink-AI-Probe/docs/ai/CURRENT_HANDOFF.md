...
- 更新时间：`2026-08-12T10:45:00+08:00`
- 分支：`feature/quick-web-entry`
- HEAD：`分支基于 7fb00d1；快速启动入口、浏览器后端端口显示和浏览器会话自动退出待提交。`
- 远端 HEAD：`GitHub master、v0.1.6 标签和正式 Release 已同步。`
- 工作树：当前功能分支保留快速启动入口与浏览器会话生命周期的未提交改动；无额外 worktree。
- 当前任务：浏览器 Web GUI 关闭后主动释放后端端口、下载器 CMD 串口和调试资源，安装版生命周期保持独立。
- 状态：`browser_session_verified`
...
4. 审查并提交 feature/quick-web-entry；合并、安装包和发布需维护者另行授权。
