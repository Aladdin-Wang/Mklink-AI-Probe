...
- 更新时间：`2026-09-03T12:05:00+08:00`
...
- 当前任务：合并 HEX 修复 21cebb8 已用客户原文件在 GD32F303CET6 验证：程序运行后自行写 BB/44 保护，用户及客户已确认每次下载需解锁。获本轮 3.3V、永久擦除与后续自主解锁授权，8 次共用 CLI 解锁成功，首轮全部 512KiB 为 FF；GUI 三连接×三复位九组烧录/校验通过。最终保留客户固件及其再次写入的保护值。额外 GUI 一键流程停在宿主原生确认框，未启动新任务，待用户处理；未重打包。STM32 13 组和既有测试证据保留。
...
- **合并 HEX 与复位连接矩阵**：重复 05 入口 HEX 共 180732B 在 GD32F303CET6 真机校验通过。客户固件自锁已由物理 BB/44、保护位、静态调用链及客户确认；先解锁再 GUI 下载的三连接×三复位九组通过，未解锁两组 PROGRAM_FAIL 留作对照。8 次 CLI 解锁使用 3.3V；首次整片 524288B 为 FF。STM32 V3 接 RST 九组与无 RST 四组也通过。GUI 一键安全流程仍待宿主弹窗解除后验证。详见 docs/verification/v0.2.0-hex-connect-modes-hil.md。
...
- **probe**：当前 MKLink V3 + GD32F303CET6，RST 已接回；新版 V3/V4 按开漏处理。客户固件已回烧，会自行写入 BB/44 保护。用户确认本轮同板每次下载前可自主解锁、永久擦除并以 3.3V 断电复位；换板或变更电压须重新确认。硬件任务已结束，GUI 仍有待处理的解锁勾选确认框。
...
1. STM32 13 组和 GD32 客户 HEX 九组分步下载矩阵已完成。用户回来先处理原生解锁确认框（取消可恢复结果页）；后续补 GUI 一键解锁下载，评估仅解锁置灰与保护错误提示。不得把客户自锁误判为连接/HEX 故障。
...
- 当前 GUI 仅勾解锁时开始按钮置灰，需 CLI 共用后端执行独立解锁。受保护 GD32 普通下载仍报泛化页编程错误，尚未增加明确保护提示。宿主原生 confirm 无浏览器句柄时不能用当前 Windows 自动化操作该宿主；额外一键解锁下载未验收。
