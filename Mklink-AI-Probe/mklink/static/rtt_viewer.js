...
var _watchWrittenValues = {};  // name -> {displayText, timeout} for write highlight
...
function _bindWatchDelegates(tbody) {
    ...
    if (el.classList.contains('watch-visible-toggle')) {
      ...
      if (el.checked) {
        ...
        if (typeof IS_SUPERWATCH_MODE !== 'undefined' && IS_SUPERWATCH_MODE) {
          fetch('/api/superwatch/remove', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:childPath})});
        }
      }
  ...
  tbody.addEventListener('click', function(e) {
    ...
    if (expandBtn) {
      ...
      if (_expandedRows[name]) {
        ...
        if (_inspectCache[baseName]) {
          ...
          fetch('/api/superwatch/inspect?name=' + encodeURIComponent(name))
            .then(function(r){ return r.json(); })
            .then(function(d){
              if (d.tree) {
    ...
    if (typeof IS_SUPERWATCH_MODE !== 'undefined' && IS_SUPERWATCH_MODE) {
      fetch('/api/superwatch/remove', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:n})});
    }
  });
...
function superwatchSearch(query) {
  ...
  fetch('/api/superwatch/search?q=' + encodeURIComponent(query))
    .then(function(r){ return r.json(); })
    .then(function(d){
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'p' || e.key === 'P') {
    if (collectionState === 'running') {
      fetch('/api/pause', {method:'POST'}).then(function(r){return r.json()}).then(function(d){updateCollectionUI(d.status)}).catch(function(){});
    } else if (collectionState === 'paused') {
      fetch('/api/resume', {method:'POST'}).then(function(r){return r.json()}).then(function(d){updateCollectionUI(d.status)}).catch(function(){});
    }
  }
